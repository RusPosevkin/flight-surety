...
contract('Flight Surety Tests', async (accounts) => {
    ...
    it('Airline --> can register 4 new airlines', async () => {
        ...
        let result4 = await config.flightSuretyApp.isAirlineRegistered.call(config.testAddresses[3]);
        assert.equal(result4, false, "Unable to register Airline 4");
    ...
    it('Airline --> can fund the 4 new airlines', async () => {
        ...
        let result4 = await config.flightSuretyApp.isAirlineActivated.call(config.testAddresses[3]);
        assert.equal(result4, false, "Unable to fund Airline 4");
    ...
    it('Airline --> can register fifth new airline that requires multi-party consensus of 50% of registered airlines', async () => {
        let result1 = await config.flightSuretyApp.isAirlineRegistered.call(config.testAddresses[5]);
        ...
        await config.flightSuretyApp.registerAirline(config.testAddresses[5], {from: config.testAddresses[1]});

        let result2 = await config.flightSuretyApp.isAirlineRegistered.call(config.testAddresses[5]);
        assert.equal(result2, true, "Unable to register Airline");

    });

    it('Airline --> can fund fifth new airline', async () => {
        let result1 = await config.flightSuretyApp.isAirlineActivated.call(config.testAddresses[5]);
        assert.equal(result1, false, "Unable to fund Airline");

        await config.flightSuretyApp.activateAirline(config.testAddresses[5], {
            from: config.firstAirline,
            value: config.weiMultiple * 10
        });

        let result5 = await config.flightSuretyApp.isAirlineActivated.call(config.testAddresses[5]);
        assert.equal(result5, true, "Unable to fund Airline");
    });
